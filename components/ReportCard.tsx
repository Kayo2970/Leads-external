...
import { FileText, Eye, CheckCircle, ArrowDownToLine, ExternalLink, Sparkles } from "lucide-react";
        ...
        <div className={`p-6 sm:p-8 group flex flex-col justify-between h-full ${lightMode ? "text-[#1E0C3D]" : "text-white"}`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
            <button
              ...
              className="w-full py-3 px-4 rounded-xl font-bold text-xs bg-white text-[#1E0C3D] hover:bg-slate-100 border border-purple-200 shadow-md flex items-center justify-center space-x-2 transition-all duration-200 hover:scale-[1.02]"
            >
              <Eye className="w-4 h-4 text-[#9C1256]" />
            ...
            <a
              href={report.fileUrl}
              download
              className="w-full py-3 px-4 rounded-xl font-bold text-xs bg-gradient-to-r from-[#9C1256] to-[#DE3F11] text-white hover:opacity-95 shadow-md flex items-center justify-center space-x-2 transition-all duration-200 hover:scale-[1.02]"
            >
